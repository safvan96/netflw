...
(function(){
  ...
  const P = [
    ...
    {n:"Magnetic Bypass Level Indicator",code:"NT-MBP",c:"level",g:"bypass",t:0,ind:["oil","chemical","water"],tags:["No Power"],s:[["200°C","Max temp"],["PN10","Pressure"]],d:"Sealed magnetic flag indication, no power, zero maintenance. For fuel, chemical and boiler-drum tanks.",pdf:"nt-mbl",img:"assets/img/bypass.png"},
    ...
    {n:"Motorized Level Sensor",code:"NT-VMS",c:"level",g:"motor",t:0,ind:["mining","food"],tags:["Solids Detection"],s:[["Relay","Output"],["IP65","Protection"]],d:"Rotary-paddle detection of bulk solids, grain silos, cement, plastic granule and powder storage.",img:"assets/img/motor.png"},
    {n:"Side-Mount Level Switch",code:"NT-MLP",c:"switch",g:"sideswitch",t:0,ind:["water","chemical","oil"],tags:["Point Level"],s:[["Side mount","Type"],["IP67","Protection"]],d:"Lever-arm float switch mounted on the side wall of a tank for high/low alarms and pump control in water, chemical and oil service.",img:"assets/products/NT-MLP.png"},
    ...
    {n:"Angle Sensor (RCS Series)",code:"NT-VKS-H 3100/2100",c:"process",g:"angle",t:0,ind:["oil"],tags:["Contactless"],s:[["0–360°","Range"],["12-bit","Resolution"]],d:"Contactless Hall-effect angle sensing with unlimited rotation life, valve position feedback and mobile machinery.",img:"assets/img/angle.png"},
  ...
  function T(key){ if(window.PM_DICT&&PM_DICT[key]){var l=L();if(l!=='en'&&PM_DICT[key][l])return PM_DICT[key][l];} return key; }
  ...
  function card(p){
    ...
    const tags=(p.tags||[]).map(t=>{
      ...
      return `<span class="${cls}" data-i18n="${t}">${T(t)}</span>`;
    }).join('');
    const specs=p.s.map(x=>`<div class="ps"><b>${x[0]}</b><span data-i18n="${x[1]}">${T(x[1])}</span></div>`).join('');
    const href=p.link?p.link:('product.html?p='+encodeURIComponent(p.code));
    const media = `<div class="glyph glyph-anim">${G[p.g]}</div>`;
    const photo = p.img ? `<img class="prod-hover-photo" src="${p.img}" alt="${T(p.n)}" loading="lazy">` : '';
    ...
    return `<a href="${href}" class="card prod-card${theme}">
      ...
      <div class="prod-body">
        <span class="prod-cat"><span class="pc-name" data-i18n="${cat[p.c]}">${T(cat[p.c])}</span>${p.code&&p.code!==', '?' · '+p.code:''}</span>
        <h3 data-i18n="${p.n}">${T(p.n)}</h3>
        <p data-i18n="${p.d}">${T(p.d)}</p>
        <div class="prod-specs">${specs}</div>
        <span class="prod-link" data-i18n="View product">${T('View product')} <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M5 12h14M13 6l6 6-6 6"/></svg></span>
